...
function checkemptyfieldRcV(pbi){
    if($.trim($('#accCodeCombinitionsH').val()) == ''){
        alert('Input Account is not valid');
        document.getElementById('sourceName').style.borderColor = "red";
        return false;
    }
    else{
        document.getElementById('sourceName').style.borderColor = "#CCC";
    }
    prevDbClickRcv(pbi);
}
