...
//    }
    ...
    public String getAllDepartments(Model model, RedirectAttributes redirectAttributes) throws Exception {
        ...
        // Extract JWT from the credentials if it's of type String
        if (credentials instanceof String) {
            jwt = (String) credentials;
        }

        try {
            // Prepare the department model and fetch all departments
            DepartmentDto departmentModel = new DepartmentDto();
            model.addAttribute("department", departmentModel);

            List<DepartmentDto> allDepartments = departmentService.getAllDepartment(jwt);
            model.addAttribute("departments", allDepartments);

        } catch (CustomException e) {
            // Handle the custom exception for expired JWT session
            System.out.println(e.getMessage());
            redirectAttributes.addAttribute("message", "Session/JWT Expired! " + e.getMessage());
            return "redirect:/login";
